...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        ...
        print("region : {}".format(region)) # prashanth

        print("store_num : {}".format(store_num)) # kiran

        # processing_date = datetime.now().strftime("%Y-%m-%d")
        # processing_hour = datetime.now().strftime("%H:%M:%S")[0:2]

        invoice_processing_date = str(event_msg["RetailTxnData"]["InvoiceDate"])[:10]
        invoice_processing_hour = str(event_msg["RetailTxnData"]["InvoiceDate"])[11:13]
        ...
        bucket_name = 'retail-dataset-training'
